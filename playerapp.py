...
load_dotenv(dotenv_path="/mnt/Hydra/Downloads/COC/.env")
...
with st.sidebar:
    ...
    env_token = os.getenv("COC_TOKEN", "")
    token = st.text_input("API Token", value=env_token, type="password")
    st.divider()
...
if app_mode == "🕵️ Player Inspector":
    ...
    # PHASE 1: FETCH DATA & STORE IN STATE
    if (inspect_btn or st.session_state.trigger_fetch) and target_tag and token:
        st.session_state.trigger_fetch = False
        with st.spinner("Infiltrating Supercell Servers..."):
            st.session_state.scanned_player = asyncio.run(process_player_inspector(target_tag, token))

    # PHASE 2: RENDER UI FROM CACHED STATE
    if st.session_state.scanned_player:
        ...
        if error:
            ...
            # THE JUMP BUTTON (Using on_click callback)
            if profile.get("clan"):
...
# ------------------------------------------
    ...
    # PHASE 1: FETCH DATA & STORE IN STATE
    if (audit_btn or st.session_state.trigger_fetch) and target_tag and token:
        st.session_state.trigger_fetch = False
        with st.spinner("Compiling Ledgers..."):
            st.session_state.scanned_clan = asyncio.run(process_clan_auditor(target_tag, input_type, token))
